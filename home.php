...
if ($query !== '') {
  if ($type === 'user') {
    $where = "WHERE u.username LIKE '%" . $conn->real_escape_string($query) . "%'";
  } else {
    $where = "WHERE p.title LIKE '%" . $conn->real_escape_string($query) . "%'";
  }
}
if ($sort === 'oldest') {
...
<body>
  <header>
    <div class="Top">
      <h1><a href="home.php">🦆</a> 전체글 모아보기</h1>
      <div class="myPage">
        <?php if ($logged_in): ?>
    ...
    <nav>
      <div class="category">
        <a href="category_free.php">자유게시판</a>
        <a href="category_2ch.php">괴담</a>
      </div>
      <div class="exTop">
        <!-- nav 검색 부분 -->
